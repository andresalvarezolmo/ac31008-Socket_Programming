...
#name of the server, port and channel we’ll be connecting to
parser = argparse.ArgumentParser(description='IRC bot parameters.')
parser.add_argument("server", help="enter the IP address of the server you are trying to connect")
parser.add_argument("port", help="enter the port")
parser.add_argument("channel", help="enter the channel")
# name of the bot
parser.add_argument("botnick", help="botnick")
...
exitcode = "Bye " + args.botnick #text to exit
afile = 'facts.txt' #file saving thr random facts available
users = [] #list with all users on the channel
user = "user"

try:
  ircsock.connect((args.server, int(args.port))) # Here we connect to the server using the port 6667
except:
  print("ERROR: Could not connect to server: " + args.server)

try:
  #send some information to let the server know who we are. 
  ircsock.send(bytes("USER "+ args.botnick +" "+ args.botnick +" "+ args.botnick + " " + args.botnick + "\n", "UTF-8")) # user information
  ircsock.send(bytes("NICK "+ args.botnick +"\n", "UTF-8")) # assign the nick to the bot
except:
  print("ERROR: Could not send to server: " + args.server)
...
def slapuser(msg, target=args.channel):
